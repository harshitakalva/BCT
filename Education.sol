...
function getCertification(uint256 index) public view returns (string memory
studentName, string memory certificateHash, uint256 timestamp) {
require(index < certifications.length, "Invalid index.");
Certification memory cert = certifications[index];
return (cert.studentName, cert.certificateHash, cert.timestamp);
}
}